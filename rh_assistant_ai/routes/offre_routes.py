...
@offre_bp.route("/questions_entretien/<int:offre_id>", methods=["GET"])
@login_required
@role_required("recruteur")
def questions_entretien(offre_id):
    # 1. Charger l'offre et vérifier la sécurité (même entreprise)
    offre = Offre.query.get_or_404(offre_id)
    if offre.recruteur.entreprise_id != current_user.entreprise_id:
        flash("Action non autorisée.", "danger")
        return redirect(url_for("recruteur.dashboard")) # Mettez votre route par défaut ici

    # 2. Récupérer la relation et parser le JSON
    questions_list = []
    # Si votre relation back_populates='questions' renvoie une liste, on prend le premier élément
    question_record = offre.questions[0] if isinstance(offre.questions, list) and offre.questions else offre.questions

    if question_record and question_record.donnees_json:
        try:
            questions_list = json.loads(question_record.donnees_json)
        except json.JSONDecodeError:
            flash("Format JSON invalide en base de données.", "danger")

    # 3. Retourner directement au template
    return render_template(
